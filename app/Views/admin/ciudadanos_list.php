            ...
            <?php if (!empty($ciudadanos)): ?>
              <?php foreach ($ciudadanos as $ciudadano): ?>
                <tr>
                  ...
                  <td><?= esc($ciudadano['nombre_usuario']) ?></td>
                  <td><?= esc($ciudadano['email']) ?></td>
                  <td><?= esc($rolesMap[$ciudadano['rol_id']] ?? 'Desconocido') ?></td>
                  <td>
                    <div class="buttons is-small">
                      <button class="button is-small is-light" title="Editar" onclick="openCitizenModal(<?= $ciudadano['id'] ?>, '<?= esc($ciudadano['nombre_usuario'], 'js') ?>', '<?= esc($ciudadano['email'], 'js') ?>')">
  ...
  <div class="modal-card">
    ...
    <section class="modal-card-body">
      <form id="citizen-form" action="<?= base_url('ciudadanos/save') ?>" method="post">
        ...
        <div class="field">
          <label class="label" for="citizen-nombre_usuario">Nombre de Usuario</label>
          <div class="control">
            <input class="input" type="text" name="nombre_usuario" id="citizen-nombre_usuario" required>
          </div>
        </div>
        <div class="field">
